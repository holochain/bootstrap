...
describe('kitsune ts-io', () => {
  ...
  it('KitsuneSpace decodes correctly', () => {
    ...
    assert.ok(
      isRight(
        Kitsune.Space.decode(Uint8Array.from(Array(Kitsune.spaceLength))),
      ),
    )

    assert.ok(
      isLeft(
        Kitsune.Space.decode(Uint8Array.from(Array(Kitsune.spaceLength - 1))),
      ),
    )
    assert.ok(
      isLeft(
  ...
  it('KitsuneAgent decodes correctly', () => {
    ...
    assert.ok(
      isRight(
        Kitsune.Agent.decode(Uint8Array.from(Array(Kitsune.agentLength))),
      ),
    )

    assert.ok(
      isLeft(
        Kitsune.Agent.decode(Uint8Array.from(Array(Kitsune.agentLength - 1))),
      ),
    )
    assert.ok(
      isLeft(
  ...
  it('KitsuneSignature decodes correctly', () => {
    ...
    assert.ok(
      isRight(
        Kitsune.Signature.decode(
          Uint8Array.from(Array(Kitsune.signatureLength)),
        ),
      ),
    )
