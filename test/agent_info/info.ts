...
describe('agent info ts-io', () => {
  ...
  it('should decode urls', () => {
    ...
    assert.ok(isRight(AgentInfo.Urls.decode(['', 'foo'])))
    assert.ok(
      isRight(AgentInfo.Urls.decode(Array(AgentInfo.MAX_URLS).fill('a'))),
    )

    assert.ok(isLeft(AgentInfo.Urls.decode('')))
    assert.ok(isLeft(AgentInfo.Urls.decode('foo')))
    assert.ok(
      isLeft(AgentInfo.Urls.decode(Array(AgentInfo.MAX_URLS + 1).fill('a'))),
    )
  })
  ...
  it('should decode times', () => {
    ...
    assert.deepEqual(AgentInfo.SignedAtMsSafe.decode(past), right(past))

    let now = AgentInfo.now()
    assert.deepEqual(AgentInfo.SignedAtMsSafe.decode(now), right(now))
     */
    ...
    */
    ...
    assert.ok(
      isRight(AgentInfo.ExpiresAfterMsSafe.decode(AgentInfo.MIN_EXPIRES)),
    )
    assert.ok(
      isRight(AgentInfo.ExpiresAfterMsSafe.decode(AgentInfo.MAX_EXPIRES)),
    )
  })
  ...
  it('should decode packed data', () => {
    ...
    assert.ok(
      isRight(AgentInfo.AgentInfoSafe.decode(MP.encode(aliceAgentVapor))),
    )
    ...
    assert.ok(
      isLeft(AgentInfo.AgentInfoSafe.decode(MP.encode(aliceEncodedCorrupted))),
    )
    ...
    assert.ok(
      isLeft(AgentInfo.AgentInfoSafe.decode(MP.encode(aliceMaliciousProperty))),
    )
  })
})
