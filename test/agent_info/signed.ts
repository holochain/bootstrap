...
import { bobSignedAlicePostBody, aliceVaporEncodedInfo, aliceVaporPutBody, aliceVaporPutBodyBadSignature } from '../fixture/requests'
...
describe('agent info signed', () => {
 ...
 it('should decode AgentInfoSignedRaw correctly', () => {
  ...
  assert.ok(isRight(agentInfoSignedRawSafe.decode(aliceVaporPutBody)))

  // any bad signature must not be valid
  assert.ok(isLeft(agentInfoSignedRawSafe.decode(aliceVaporPutBodyBadSignature)))
 })

 it('should decode agentInfoSigned correctly', () => {
  assert.ok(isRight(agentInfoSignedSafe.decode(aliceVaporPutBody)))
